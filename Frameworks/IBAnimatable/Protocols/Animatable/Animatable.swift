...
public extension Animatable where Self: UIView {
    ...
    {
        AnimationPromise(view: self).delay(delay).then(animation, duration: duration, damping: damping, velocity: velocity, force: force)
    }
    ...
    internal func doAnimation(_ animation: AnimationType? = nil, configuration: AnimationConfiguration, promise: AnimationPromise<Self>) {
        ...
        doAnimation(animation ?? animationType, configuration: configuration, completion: completion)
    }
...
private extension UIView {
    func doAnimation(_ animation: AnimationType, configuration: AnimationConfiguration, completion: @escaping () -> Void) {
        ...
        case .slideFade(let way, let direction):
        ...
        case .compound(let animations, let run):
            ...
            case .parallel:
                ...
                for animation in animations {
                    doAnimation(animation, configuration: configuration, completion: finalCompletion)
                }
            }
        case .none:
            break
    ...
    var screenSize: CGSize {
        window?.screen.bounds.size ?? .zero
    }
}
...
public extension Animatable where Self: UIBarItem {
                 ...
                 completion: AnimatableCompletion? = nil)
    {
        ...
        view.doAnimation(animation ?? animationType, configuration: configuration) {
            completion?()
        }
    }
}
