...
public protocol BorderDesignable: AnyObject {
    /**
...
extension BorderDesignable {
    ...
    private var borderThickness: (active: CGFloat, inactive: CGFloat) {
        (2, borderWidth)
    }
    
    var isActiveBorder: Bool {
        guard let `self` = self as? UITextField else { return false }
        
        return activeColor != nil && `self`.isFirstResponder
    }
...
extension BorderDesignable {
    ...
    private func drawBordersSides(in view: UIView) {
        ...
        shapeLayer.strokeColor = isActiveBorder ? activeColor!.cgColor : borderColor!.cgColor
        shapeLayer.lineWidth = isActiveBorder ? borderThickness.active : borderThickness.inactive
        shapeLayer.frame = view.bounds
        switch borderType {
        case .dash(let dashLength, let spaceLength):
