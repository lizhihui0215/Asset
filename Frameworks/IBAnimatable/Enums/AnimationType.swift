...
public extension AnimationType {
    static func scaleTo(x: Double, y: Double) -> AnimationType {
        .scale(fromX: 1, fromY: 1, toX: x, toY: y)
    }

    static func scaleFrom(x: Double, y: Double) -> AnimationType {
        .scale(fromX: x, fromY: y, toX: 1, toY: 1)
    }
}
...
extension AnimationType: ExpressibleByArrayLiteral {
    ...
    public init(arrayLiteral elements: AnimationType...) {
        if let first = elements.first {
            if elements.count == 1 {
                self = first
            } else if case .compound(let animation, .sequential) = first {
