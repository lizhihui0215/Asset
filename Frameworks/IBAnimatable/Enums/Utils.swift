...
extension Array {
    ...
    subscript(safe index: Int) -> Element? {
        indices.contains(index) ? self[index] : nil /// Returns the element at the specified index iff it is within bounds, otherwise nil.
    }
}
...
extension Array where Element: Udra.Node {
    ...
    func toDoubles() -> [Double] {
        let nodes = compactMap { $0 as? Udra.NumberNode }
...
extension RawRepresentable {
    ...
    func iterateEnum<T: CaseIterable>(from: T.Type) -> AnyIterator<T> {
        AnyIterator(T.allCases.makeIterator())
    }
#else
...
extension String {
    func parseNameAndParams() -> [Udra.Node] {
        let parser = Udra.Parser(tokens: Udra.Lexer.tokenize(lowercased()))
        do {
    ...
    public func parse<T: IBEnum>(default defaultValue: T) -> T {
        T(string: self) ?? defaultValue
    }
}

extension NSRegularExpression {
    func matched(_ string: String) -> (String, CountableRange<Int>)? {
        let range = rangeOfFirstMatch(in: string, options: [], range: NSRange(0 ..< string.utf16.count))
