...
class Udra { // swiftlint:disable:this type_body_length
    ...
    class NumberNode: Node {
        ...
        static func + (lhs: NumberNode, rhs: NumberNode) -> NumberNode {
            NumberNode(value: lhs.value + rhs.value)
        }
    }
    ...
    class BinaryOperatorNode: Node {
        ...
        override var description: String {
            "\(type(of: self))(name: \"\(name)\", lhs: \(lhs), rhs: \(rhs))"
        }

        func evaluate() -> Any? {
            self.operator.evaluate(lhs: lhs, rhs: rhs)
        }
    }
    ...
    class CallNode: Node {
        ...
        override var description: String {
            "\(type(of: self))(name: \"\(name)\", arguments: \(arguments))"
        }
    }
    ...
    class Parser {
        ...
        func parseNumber() throws -> Node {
            guard case .number(let value, _) = popCurrentToken() else {
        ...
        func currentTokenPrecedence() throws -> Int {
            ...
            if case .undefined(let op, _) = currentToken() {
                throw ParseError.undefinedOperator(op)
            }

            guard case .operator(let op, _) = currentToken() else {
                return -1
            }
        ...
        func parseBinaryOperator(node: Node, exprPrecedence: Int = 0) throws -> Node {
            ...
            while true {
                ...
                guard case .operator(let op, _) = popCurrentToken() else {
                    throw ParseError.unexpectToken
                }
