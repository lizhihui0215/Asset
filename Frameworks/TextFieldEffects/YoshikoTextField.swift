...
@IBDesignable open class YoshikoTextField: TextFieldEffects {
     ...
     */
    @IBInspectable open dynamic var placeholderColor: UIColor = .darkGray {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    private func rectForBounds(_ bounds: CGRect) -> CGRect {
        CGRect(x: bounds.origin.x, y: bounds.origin.y + placeholderHeight, width: bounds.size.width, height: bounds.size.height - placeholderHeight)
    }

    private var placeholderHeight: CGFloat {
        placeHolderInsets.y + placeholderFontFromFontAndPercentageOfOriginalSize(font: font!, percentageOfOriginalSize: placeholderFontScale).lineHeight
    }
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        bounds.offsetBy(dx: textFieldInsets.x, dy: textFieldInsets.y + placeholderHeight / 2)
    }
