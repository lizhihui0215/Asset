...
@class WwGCDAsyncReadPacket;
@class WwGCDAsyncWritePacket;
@class WwGCDAsyncSocketPreBuffer;
@protocol WwGCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const WwGCDAsyncSocketException;
extern NSString *const WwGCDAsyncSocketErrorDomain;

extern NSString *const WwGCDAsyncSocketQueueName;
extern NSString *const WwGCDAsyncSocketThreadName;

extern NSString *const WwGCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const WwGCDAsyncSocketUseCFStreamForTLS;
#endif
#define WwGCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define WwGCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define WwGCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const WwGCDAsyncSocketSSLPeerID;
extern NSString *const WwGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const WwGCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const WwGCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const WwGCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const WwGCDAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const WwGCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define WwGCDAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, WwGCDAsyncSocketError) {
	WwGCDAsyncSocketNoError = 0,           // Never used
	WwGCDAsyncSocketBadConfigError,        // Invalid configuration
	WwGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	WwGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	WwGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	WwGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	WwGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	WwGCDAsyncSocketClosedError,           // The remote peer closed the connection
	WwGCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface WwGCDAsyncSocket : NSObject
...
- (instancetype)initWithDelegate:(nullable id<WwGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<WwGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<WwGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<WwGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<WwGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<WwGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<WwGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<WwGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<WwGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
@protocol WwGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(WwGCDAsyncSocket *)sock;
...
**/
- (void)socket:(WwGCDAsyncSocket *)sock didAcceptNewSocket:(WwGCDAsyncSocket *)newSocket;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)socket:(WwGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(WwGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(WwGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(WwGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(WwGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(WwGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(WwGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(WwGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(WwGCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(WwGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socketDidSecure:(WwGCDAsyncSocket *)sock;
...
**/
- (void)socket:(WwGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
