...
- (nonnull instancetype)initWithSessionPreset:(enum CaptureSessionPreset)sessionPreset focusMode:(enum FocusMode)focusMode exposureMode:(enum ExposureMode)exposureMode flashMode:(enum FlashMode)flashMode videoExportType:(enum VideoExportType)videoExportType OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@interface ZLEditImageViewController (SWIFT_EXTENSION(ZLPhotoBrowser))
@end

typedef SWIFT_ENUM(NSInteger, EditImageTool, closed) {
  EditImageToolDraw = 0,
  EditImageToolClip = 1,
  EditImageToolImageSticker = 2,
  EditImageToolTextSticker = 3,
  EditImageToolMosaic = 4,
  EditImageToolFilter = 5,
};
...
/// \param datas Must be one of PHAsset, UIImage and URL, will filter ohers in init function.
///
...
/// warning:
/// The video can only be edited when no photos are selected, or only one video is selected, and the selection callback is executed immediately after editing is completed.
